...
static rct_tile_element* map_get_footpath_element_slope(int32_t x, int32_t y, int32_t z, int32_t slope)
{
    ...
    do
    {
        if (tileElement->GetType() == TILE_ELEMENT_TYPE_PATH && tileElement->base_height == z
            ...
            && (tileElement->AsPath()->GetPathDirection() == (slope & FOOTPATH_PROPERTIES_SLOPE_DIRECTION_MASK)))
        {
            return tileElement;
        }
    } while (!(tileElement++)->IsLastForTile());
...
static void loc_6A6620(int32_t flags, int32_t x, int32_t y, rct_tile_element* tileElement)
{
    ...
    {
        int32_t direction = tileElement->AsPath()->GetPathDirection();
    ...
    int32_t type, int32_t x, int32_t y, int32_t z, int32_t slope, int32_t flags, uint8_t pathItemType)
{
    ...
    if (flags & GAME_COMMAND_FLAG_APPLY)
    {
        ...
        else
        {
            ...
            pathElement->SetPathDirection(slope & FOOTPATH_PROPERTIES_SLOPE_DIRECTION_MASK);
            if (slope & FOOTPATH_PROPERTIES_FLAG_IS_SLOPED)
...
static rct_tile_element* footpath_get_element(int32_t x, int32_t y, int32_t z0, int32_t z1, int32_t direction)
{
    ...
    do
    {
        ...
        if (z1 == tileElement->base_height)
        {
            ...
            {
                slope = tileElement->AsPath()->GetPathDirection();
                if (slope != direction)
                    break;
            }
            return tileElement;
        ...
        if (z0 == tileElement->base_height)
        {
            ...
            slope = tileElement->AsPath()->GetPathDirection() ^ 2;
            if (slope != direction)
                break;
...
static bool footpath_disconnect_queue_from_path(int32_t x, int32_t y, rct_tile_element* tileElement, int32_t action)
{
    ...
    if (action < 0)
    {
        uint8_t direction = tileElement->AsPath()->GetPathDirection();
    ...
    {
        if ((action < 0) && (direction == tileElement->AsPath()->GetPathDirection()))
            continue;
        if (sub_footpath_disconnect_queue_from_path(x, y, tileElement, action, direction))
            return true;
    }
        ...
        do
        {
            switch (tileElement->GetType())
            {
                case TILE_ELEMENT_TYPE_PATH:
                    ...
                    {
                        if (tileElement->AsPath()->IsSloped() && tileElement->AsPath()->GetPathDirection() != direction)
                        {
                            return;
                        }
                        else
                    ...
                    {
                        if (!tileElement->AsPath()->IsSloped() || tileElement->AsPath()->GetPathDirection() != (direction ^ 2))
                        {
                            return;
                        }
                        goto loc_6A6F1F;
    ...
    rct_neighbour_list* neighbourList)
{
    ...
    if (tileElement->GetType() == TILE_ELEMENT_TYPE_PATH)
    {
        if (tileElement->AsPath()->IsSloped())
        {
            if ((tileElement->AsPath()->GetPathDirection() - direction) & 1)
            {
                return;
            }
            if (tileElement->AsPath()->GetPathDirection() == direction)
            {
                z += 2;
            }
        }
    ...
    for (;;)
    {
        ...
        do
        {
            ...
            if (tileElement->base_height == z - 2)
            {
                ...
                if ((tileElement->AsPath()->GetPathDirection() ^ 2) != direction)
                    break;
    ...
    int32_t junctionTolerance)
{
    ...
    do
    {
        ...
        if (tileElement->AsPath()->IsSloped() && (slopeDirection = tileElement->AsPath()->GetPathDirection()) != direction)
        {
...
uint8_t PathElement::GetPathDirection() const
{
    return entryIndex & FOOTPATH_PROPERTIES_SLOPE_DIRECTION_MASK;
}

void PathElement::SetPathDirection(uint8_t newSlope)
{
...
static void footpath_remove_edges_towards(int32_t x, int32_t y, int32_t z0, int32_t z1, int32_t direction, bool isQueue)
{
    ...
    do
    {
        ...
        if (z1 == tileElement->base_height)
        {
            if (tileElement->AsPath()->IsSloped())
            {
                uint8_t slope = tileElement->AsPath()->GetPathDirection();
                if (slope != direction)
                    break;
            }
        ...
        if (z0 == tileElement->base_height)
        {
            ...
            uint8_t slope = tileElement->AsPath()->GetPathDirection() ^ 2;
            if (slope != direction)
                break;
            ...
            case TILE_ELEMENT_TYPE_PATH:
                if (tileElement->base_height == coords.z)
                {
                    if (!tileElement->AsPath()->IsSloped())
                        ...
                        return true;
                    else if (tileElement->AsPath()->GetPathDirection() == (coords.direction ^ 2))
                        // The footpath is sloped and its lowest point matches the edge connection
                        return true;
                }
                else if (tileElement->base_height + 2 == coords.z)
                {
                    if (tileElement->AsPath()->IsSloped() && tileElement->AsPath()->GetPathDirection() == coords.direction)
...
void footpath_remove_edges_at(int32_t x, int32_t y, rct_tile_element* tileElement)
{
    ...
    for (uint8_t direction = 0; direction < 4; direction++)
    {
        ...
        if (tileElement->GetType() == TILE_ELEMENT_TYPE_PATH)
        {
            if (tileElement->AsPath()->IsSloped())
            {
                int32_t slope = tileElement->AsPath()->GetPathDirection();
