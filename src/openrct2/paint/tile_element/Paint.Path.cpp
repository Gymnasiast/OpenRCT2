    ...
    rct_footpath_entry* footpathEntry, uint32_t base_image_id, uint32_t imageFlags)
{
    if (tile_element->AsPath()->IsQueue())
    {
        ...
        {
            switch ((tile_element->AsPath()->GetPathDirection() + session->CurrentRotation)
        ...
        if (tile_element->AsPath()->IsSloped())
        {
            if (tile_element->AsPath()->GetPathDirection() == direction)
                height += 16;
        }
    ...
    rct_footpath_entry* footpathEntry, uint32_t imageFlags, uint32_t sceneryImageFlags, bool word_F3F038)
{
    ...
    uint8_t direction = (tile_element->AsPath()->GetPathDirection() + session->CurrentRotation)
        & FOOTPATH_PROPERTIES_SLOPE_DIRECTION_MASK;
...
void path_paint(paint_session* session, uint16_t height, const rct_tile_element* tile_element)
{
    ...
    else
    {
        if (tile_element->AsPath()->IsSloped())
        {
            ...
            if (surface->AsSurface()->GetSlope() != byte_98D800[tile_element->AsPath()->GetPathDirection()])
            {
                word_F3F038 = true;
            }
        }
    ...
    if (gStaffDrawPatrolAreas != 0xFFFF)
    {
        ...
        if (staff_is_patrol_area_set(200 + staffType, x, y))
        {
            ...
            if (tile_element->AsPath()->IsSloped())
            {
                imageId = 2619 + ((tile_element->AsPath()->GetPathDirection() + session->CurrentRotation) & 3);
                height2 += 16;
            }
    ...
    bool hasFences, uint32_t imageFlags, uint32_t sceneryImageFlags)
{
    ...
    if (tileElement->AsPath()->IsSloped())
    {
        imageId = ((tileElement->AsPath()->GetPathDirection() + session->CurrentRotation)
                   & FOOTPATH_PROPERTIES_SLOPE_DIRECTION_MASK)
            + 16;
    }
    else
    ...
    else
    {
        ...
        if (tileElement->AsPath()->IsSloped())
        {
            image_id = ((tileElement->AsPath()->GetPathDirection() + session->CurrentRotation)
                        & FOOTPATH_PROPERTIES_SLOPE_DIRECTION_MASK)
    ...
    if (tileElement->AsPath()->IsSloped())
    {
        ax = ((tileElement->AsPath()->GetPathDirection() + session->CurrentRotation) & 0x3) + 1;
    }
    ...
    bool hasFences, uint32_t imageFlags, uint32_t sceneryImageFlags)
{
    ...
    if (tileElement->AsPath()->IsSloped())
    {
        imageId = ((tileElement->AsPath()->GetPathDirection() + session->CurrentRotation) & 3) + 16;
    }
    else
    {
        imageId = byte_98D6E0[edi];
    ...
    else
    {
        ...
        if (tileElement->AsPath()->IsSloped())
        {
            bridgeImage = ((tileElement->AsPath()->GetPathDirection() + session->CurrentRotation)
                           & FOOTPATH_PROPERTIES_SLOPE_DIRECTION_MASK)
