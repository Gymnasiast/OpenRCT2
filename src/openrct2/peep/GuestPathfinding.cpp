...
static uint8_t footpath_element_next_in_direction(TileCoordsXYZ loc, rct_tile_element* tileElement, uint8_t chosenDirection)
{
    ...
    {
        if (tileElement->AsPath()->GetPathDirection() == chosenDirection)
        {
            loc.z += 2;
        }
    }
    ...
    TileCoordsXYZ loc, rct_tile_element* inputTileElement, uint8_t chosenDirection, uint8_t* outRideIndex)
{
    ...
    {
        if (inputTileElement->AsPath()->GetPathDirection() == chosenDirection)
        {
            loc.z += 2;
        }
    }
 ...
 * Continue to the next map element without updating the parameters (best result so far). */
        ...
        do
        {
            ...
            if (tileElement->AsPath()->IsSloped() && tileElement->AsPath()->GetPathDirection() == next_test_edge)
            {
                height += 2;
            }
#if defined(DEBUG_LEVEL_2) && DEBUG_LEVEL_2
...
int32_t peep_pathfind_choose_direction(TileCoordsXYZ loc, rct_peep* peep)
{
    ...
    if (edges & ~(1 << chosen_edge))
    {
        ...
        for (int32_t test_edge = chosen_edge; test_edge != -1; test_edge = bitscanforward(edges))
        {
            ...
            if (first_tile_element->AsPath()->IsSloped() && first_tile_element->AsPath()->GetPathDirection() == test_edge)
            {
                height += 0x2;
            }
    ...
    while (true)
    {
        ...
        do
        {
            ...
            if (baseZ - 2 == tileElement->base_height)
            {
                ...
                if (tileElement->AsPath()->GetPathDirection() != (direction ^ 2))
                    break;
