    ...
    static void DrawMonths(rct_drawpixelinfo* dpi, const uint8_t* history, int32_t count, int32_t baseX, int32_t baseY)
    {
        ...
        for (i = count - 1; i >= 0; i--)
        {
            if (history[i] != 255 && yearOver32 % 4 == 0)
            {
                ...
                DrawTextBasic(
                    dpi, screenCoords - ScreenCoordsXY{ 0, 10 }, STR_GRAPH_LABEL, ft, COLOUR_BLACK, TextAlignment::CENTRE);
...
namespace Graph
{
    static void DrawMonths(rct_drawpixelinfo* dpi, const money32* history, int32_t count, int32_t baseX, int32_t baseY)
    {
        ...
        for (i = count - 1; i >= 0; i--)
        {
            if (history[i] != MONEY32_UNDEFINED && yearOver32 % 4 == 0)
            {
                ...
                DrawTextBasic(
                    dpi, screenCoords - ScreenCoordsXY{ 0, 10 }, STR_GRAPH_LABEL, &monthFormat, COLOUR_BLACK,
        ...
        const int32_t modifier, const int32_t offset)
    {
        ...
        DrawTextBasic(
            dpi, info.coords - ScreenCoordsXY{ 0, 16 }, STR_FINANCES_SUMMARY_EXPENDITURE_VALUE, &info.money, COLOUR_BLACK,
            TextAlignment::CENTRE);
