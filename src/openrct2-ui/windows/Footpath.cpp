...
static void window_footpath_place_path_at_point(int32_t x, int32_t y)
{
    ...
    switch (interactionType)
    {
        case VIEWPORT_INTERACTION_ITEM_TERRAIN:
            ...
            break;
        case VIEWPORT_INTERACTION_ITEM_FOOTPATH:
            currentType = tileElement->AsPath()->GetPathDirection();
            if (tileElement->AsPath()->IsSloped())
            {
...
static void window_footpath_start_bridge_at_point(int32_t screenX, int32_t screenY)
{
    ...
    else
    {
        ...
        if (tileElement->GetType() == TILE_ELEMENT_TYPE_PATH)
        {
            if (tileElement->AsPath()->IsSloped())
            {
                if (direction == (tileElement->AsPath()->GetPathDirection()))
                {
                    z += 2;
                }
            }
...
static void footpath_remove_tile_element(rct_tile_element* tileElement)
{
    ...
    if (tileElement->AsPath()->IsSloped())
    {
        uint8_t slopeDirection = tileElement->AsPath()->GetPathDirection();
        slopeDirection ^= 2;
    ...
    do
    {
        if (tileElement->GetType() == TILE_ELEMENT_TYPE_PATH)
        {
            if (tileElement->base_height == z)
            {
                if (tileElement->AsPath()->IsSloped())
                {
                    if (((tileElement->AsPath()->GetPathDirection()) ^ 2) != gFootpathConstructDirection)
                    {
                        continue;
                    }
                }
            ...
            else if (tileElement->base_height == zLow)
            {
                if (!tileElement->AsPath()->IsSloped())
                {
                    if ((tileElement->AsPath()->GetPathDirection()) == gFootpathConstructDirection)
                    {
                        continue;
                    }
                }
