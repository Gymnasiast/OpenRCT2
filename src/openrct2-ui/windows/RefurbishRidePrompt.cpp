    ...
    public:
        ...
        void onDraw(Drawing::RenderTarget& rt) override
        {
            ...
            if (currentRide != nullptr)
            {
                ...
                money64 refurbishCost = _demolishRideCost / 2;
                auto string = FormatStringID(stringId, currentRide->getName().c_str(), refurbishCost);

                ScreenCoordsXY stringCoords(windowPos.x + kWindowSize.width / 2, windowPos.y + (kWindowSize.height / 2) - 3);
                DrawTextWrapped(rt, stringCoords, kWindowSize.width - 4, string, { TextAlignment::centre });
            }
        }
    };
