...
static void window_land_paint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    if (gLandToolSize > MAX_TOOL_SIZE_WITH_SPRITE)
    {
        ...
        DrawTextBasic(
            dpi, screenCoords - ScreenCoordsXY{ 0, 2 }, STR_LAND_TOOL_SIZE_VALUE, &gLandToolSize, COLOUR_BLACK,
    ...
    if (!(gParkFlags & PARK_FLAGS_NO_MONEY))
    {
        ...
        if (gLandToolRaiseCost != MONEY32_UNDEFINED && gLandToolRaiseCost != 0)
            DrawTextBasic(dpi, screenCoords, STR_RAISE_COST_AMOUNT, &gLandToolRaiseCost, COLOUR_BLACK, TextAlignment::CENTRE);
        screenCoords.y += 10;

        // Draw lower cost amount
        if (gLandToolLowerCost != MONEY32_UNDEFINED && gLandToolLowerCost != 0)
            DrawTextBasic(dpi, screenCoords, STR_LOWER_COST_AMOUNT, &gLandToolLowerCost, COLOUR_BLACK, TextAlignment::CENTRE);
        screenCoords.y += 50;
        ...
        if (price != 0)
        {
            ...
            DrawTextBasic(dpi, screenCoords, STR_COST_AMOUNT, ft, COLOUR_BLACK, TextAlignment::CENTRE);
        }
    }
}
